...
if (! function_exists('days_differance')) {
  function days_differance($f_dt = '', $t_dt = '') { $days = '';
    if (empty($t_dt)) $t_dt = date("Y-m-d H:i:s");
    if ((!empty($f_dt)) && (!empty($t_dt))) { if ($f_dt <= $t_dt) {
      $diff = date_diff(date_create($f_dt), date_create($t_dt));
      $days = $diff->days;
    } }
    return $days;
  }
...
if (! function_exists('get_settings')) {
  function get_settings($key = '') { $CI  =&  get_instance();
    ...
    $CI->db->select('m_app_value');
    $CI->db->where('m_app_key',$key);
    $sql=$CI->db->get('master_app_settings');
  
  if($sql->num_rows() == 1){ return $sql->result()[0]->m_app_value; }else{ return ''; }
